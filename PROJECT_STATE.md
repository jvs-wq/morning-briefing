...
### Changelog — 2026-07-01 (LaunchAgent + monitor reconciliation)
**Trigger.** Jeff asked to reconcile everything after a 6-vs-7 agent discrepancy surfaced between the docs and live state. Full inventory (launchctl vs `~/Library/LaunchAgents/` vs repo `launchd/`) exposed two real drifts, not a simple miscount.
**Removed the orphaned `com.briefing.verify_schedule` agent.** It was a self-described **one-shot** meant to run once on 2026-05-18 and then self-delete its own plist; it never self-removed, so it lingered (next fire would have been 2027-05-18). It also (a) delivered via **iMessage** — against the v2.6 email-only rule the project is guarded to enforce — and (b) health-checked eight agents including three `com.lunarcrush.*` jobs retired 2026-06-12. Unloaded + deleted the installed plist and `git rm`'d `scripts/verify_schedule.sh` (backups saved to the session scratchpad; recoverable from git history).
**Restored the `com.briefing.lunarcrush` agent (Jeff's call).** The Sunday social/sentiment brief mode is fully implemented in code (`morning_briefing.py lunarcrush` → `run_lunarcrush_brief()`), but its LaunchAgent had gone missing (never committed to `launchd/`). Recreated `launchd/com.briefing.lunarcrush.plist` (Sunday 5:30 PM PT, `Weekday=0`, logs to `/tmp/briefing-lunarcrush.log`), installed + loaded it. Live state is back to **seven agents**: deploy, morning, premarket, recap, lunarcrush, weekend_preview, monitor.
**Fixed the monitor false-positive root cause.** `briefing_monitor.py` `WORKFLOW_LOG_PATHS` had pinned `lunarcrush` to `/tmp/lunarcrush-evening-launchd.log` — the *sibling* project's log, retired 2026-06-12 — so the Sunday-evening health check had been reading a dead file (and would false-alarm) regardless of whether this project's brief ran. Removed the override so the monitor uses the default `/tmp/briefing-lunarcrush.log` that the plist actually writes; also corrected the stale "daily evening" age comment (it's Sunday-only, gated by `_modes_for_current_time`).
**Fixed `deploy.sh` reload loop.** It had drifted to `morning premarket recap weekend_preview monitor` (lunarcrush dropped when the agent vanished); restored to the documented six non-deploy agents (`morning premarket recap lunarcrush weekend_preview monitor`).
**Doc corrections.** CLAUDE.md said "six LaunchAgents" (it never listed lunarcrush) → now seven. Clarified in CLAUDE.md + README that `deploy.sh` syncs **code + docs only** and does **not** mirror/install plists (the earlier "step 4b plist mirroring" was never present in the actual `deploy.sh`); plists are installed via `launchd/install.sh`, the single source of truth in `launchd/`. Removed the deleted `verify_schedule.sh` from the README file listing and the stale "only 3 of 7 plists mirrored" structural-gap note.
**Verified.** `briefing_monitor.py` compiles; `plutil -lint` clean on the new plist; `launchctl list` shows the seven agents with no `verify_schedule`. The live lunarcrush brief was **not** test-fired (would send a real email); dispatch path confirmed by code inspection.
