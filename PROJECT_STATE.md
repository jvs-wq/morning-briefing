...
**Status:** Production. **v2.8 readability + visual-identity pass is in the Drive edit surface, NOT yet deployed** (awaiting the next 4:50 AM auto-sync or a manual `deploy.sh --reload`). v2.7.5 model-currency fix + retired-model guard remains LIVE in prod. Two audits (news-source freshness, API efficiency) completed 2026-06-26 — findings below carry open recommendations not yet implemented.
...
3. **Ticker-box restyle (`_format_snapshot_table`, `_format_movers_table`).** The header rows of the Market Snapshot and Pre-Market Movers tables changed from tan (`#ebe7e1`) to black (`#1a1a1a`) with white text, matching the masthead banner; added a `border-top: 3px solid #c0392b` red accent to each table to echo the masthead's red rule. Appendix/detailed-data tables intentionally left tan — establishes a deliberate hierarchy (black = headline data boxes, tan = appendix). Equivalent tables in the other three briefs NOT yet propagated — left for Jeff's confirmation after he sees the morning sample.
