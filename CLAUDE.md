...
**Model currency (v2.7.5, 2026-06-23).** AI model is pinned to `claude-sonnet-4-6`. The prior pin `claude-sonnet-4-20250514` retired 2026-06-15 and 404'd every AI call, silently degrading the briefing (FILTERED NEWS showed `[???]`/`FYI` everywhere). Now every AI `except` block prints a loud `!!!! MODEL RETIRED OR INVALID` banner on a 404 model error, and the news fallback leads with a visible `SYSTEM`/`URGENT` "update the model ID" banner. **If you see that banner, swap the model ID in all 7 spots** (`grep -rn 'model="claude' *.py`) to a current model from https://platform.claude.com/docs/en/about-claude/models.
